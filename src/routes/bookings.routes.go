...
func RegisterBookingsRoutes(r *mux.Router, db *sqlx.DB, clerkClient clerk.Client) {
	...
	r.Handle("/{houseId}", controllers.CreateBooking(db, clerkClient)).Methods("POST")
	...
	r.Handle("/{houseId}/bookings", controllers.GetHouseBookings(db, clerkClient)).Methods("GET")
